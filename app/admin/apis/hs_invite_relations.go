...
// @Summary 获取邀请统计列表
// @Description 获取邀请统计列表，显示邀请人ID、邀请码、邀请人数、邀请累计金额
// @Tags 用户邀请关系冗余表
// @Param inviterUserId query string false "邀请人ID"
...
func (e HsInviteRelations) GetStatsPage(c *gin.Context) {
    req := dto.HsInviteRelationsStatsGetPageReq{}
    s := service.HsInviteRelations{}
    err := e.MakeContext(c).
        MakeOrm().
        Bind(&req).
	...
	list := make([]dto.HsInviteRelationsStatsResp, 0)
	var count int64

	err = s.GetStatsPage(&req, &list, &count)
	if err != nil {
		e.Error(500, err, fmt.Sprintf("获取邀请统计列表失败，\r\n失败信息 %s", err.Error()))
        return
	}
